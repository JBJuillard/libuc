...
** ut_memcpy_s.c
**
** Unit tests for memcpy_s function of Undefined-C library
**
...
int	ut_memcpy_s_interface(int N)
{
	...
	test_t			tdat[6] = {	{(const void *)(NULL), 128, (void *)(dst), 128, (errno_t)(EINVAL), EINVAL},
								...
								{(const void *)(src), 128, (void *)(dst), 128, (errno_t)(0), 0} };
	...
	while (i < 6)
	{
		...
		if (memcpy_s((tdat[i]).dst, (tdat[i]).smax, (tdat[i]).src, (tdat[i]).size) != (tdat[i]).ret
			|| errno != (tdat[i]).err)
			return (err);
		--err;
		++i;
...
int	ut_memcpy_s_validity(int N)
{
	...
	memset((void *)(dst), 0, N);
	if (memcpy_s((void *)(dst), (rsize_t)(N), (void *)(src), (size_t)(N)) != (errno_t)(0)
		|| errno)
	{
		free((void *)(src));
		free((void *)(dst));
