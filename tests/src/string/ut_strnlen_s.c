...
** ut_strnlen_s.c
**
** Unit tests for strnlen_s function of Undefined-C library
**
...
int	ut_strnlen_s_interface(int N)
{
	...
	while (i < 3)
	{
		if (strnlen_s((tdat[i]).str, (tdat[i]).len) != (tdat[i]).ret
			|| (tdat[i]).err != errno)
			return (err);
		++i;
		--err;
...
{
	return (ut_strnlen_s_validity(N));
}
