/*
** Units tests of slst_nsrtp function for Undefined-C library
**
...
int	ut_slst_nsrtp_interface(int N)
{
	...
	int				i;
	int				j;
	static test_t	ut_list[8] = {	{0,1,1,1,sizeof(long long),0,EINVAL,1},
	...
	while (i < 8)
	{
		...
		while (j < N)
		{
			...
			if ((ut_list[i]).ptr)
				ret = slst_nsrtp(lptr, elm, lst);
			else
				ret = slst_nsrtp(lptr, elm, (slst_t *)(NULL));
	...
	*((int *)(lst->key)) = i;
	...
	while (i < N)
	{
		...
		*((int *)(tmp->key)) = i;
		...
		errno = 0;
		if (slst_nsrtp(&lst, tmp, lst) == (slst_t *)(NULL) || errno)
		{
			free((void *)(tmp));
			err = errno;
			slst_purge(&lst, &_ofree);
