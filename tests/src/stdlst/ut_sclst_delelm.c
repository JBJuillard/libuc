...
** ut_sclst_delelm.c
**
** Unit tests for sclst_delelm function of Undefined-C library
**
...
int	ut_sclst_delelm_interface(int N)
{
	...
	while (i < 4)
	{
		...
		while (j < N)
		{
			...
			if ((ut_list[i]).elm)
			{
				if ((elm = sclst_newelm()) == (sclst_t *)(NULL)
				...
				*((int *)(elm->key)) = j;
				...
				elm = (sclst_t *)(NULL);
			sclst_delelm(elm, (void (*)(void *, size_t))((ut_list[i]).f));
...
int	ut_sclst_delelm_memchk(int N)
{
	...
	while (i < N)
	{
		if ((ptr = (sclst_t *)malloc(sizeof(slst_t))) == (sclst_t *)(NULL)
...
{
	return (ut_sclst_delelm_memchk(N));
}
