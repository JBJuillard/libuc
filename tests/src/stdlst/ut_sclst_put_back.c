...
** ut_sclst_put_back.c
**
** Unit tests for sclst_put_back function of Undefined-C library
**
...
int	ut_sclst_put_back_interface(int N)
{
	...
	while (i < 6)
	{
		...
		while (j < N)
		{
			...
			errno = 0;
			ret = sclst_put_back(lptr, k, (ut_list[i]).size);
...
int	ut_sclst_put_back_memchk(int N)
{
	...
	while (i < N)
	{
		...
		*((long long *)(k)) = i;
		errno = 0;
		if (sclst_put_back(&lst, k, sizeof(long long)) == (slst_t *)(NULL) || errno)
		{
			free(k);
			sclst_purge(&lst, &_ofree);
			if (!errno)
...
{
	return (ut_sclst_put_back_memchk(N));
}
