...
** ut_strtoull.c
**
** Unit tests for strtoull function of Undefined-C library
**
...
int	ut_strtoull_interface(int N)
{
	...
	while (i < 4)
	{
		...
		if (strtoull((tdat[i]).str, (tdat[i]).end, 10) != (tdat[i]).ret
			|| (tdat[i]).endval != end
			|| errno != (tdat[i]).error)
			return (err);
		i++;
...
int	ut_strtoull_validity(int N)
{
	...
	{
		errno = 0;
		if (!(s = ulltostr(i, 10, 0)))
			return (-1);
		if (strtoull((const char *)(s), (char **)(NULL), 10) != i
			|| errno)
			return (i + 1);
		i++;
	}
