...
** ut_strtou.c
**
** Unit tests for strtou function of Undefined-C library
**
...
int	ut_strtou_interface(int N)
{
	...
	while (i < 4)
	{
		...
		if (strtou((tdat[i]).str, (tdat[i]).end, 10) != (tdat[i]).ret
			|| (tdat[i]).endval != end
			|| errno != (tdat[i]).error)
			return (err);
		i++;
...
int	ut_strtou_validity(int N)
{
	...
	while (i < (unsigned int)(N))
	{
		...
		if (!(s = utostr(i, 10, 0)))
			return (-1);
		if (strtou((const char *)(s), (char **)(NULL), 10) != i
			|| errno)
			return (i + 1);
		i++;
	}
