...
** ut_strtol.c
**
** Unit tests for strtol function of Undefined-C library
**
...
int	ut_strtol_interface(int N)
{
	...
	while (i < 5)
	{
		...
		if (strtol((tdat[i]).str, (tdat[i]).end, 10) != (tdat[i]).ret
			|| end != (tdat[i]).endval
			|| errno != (tdat[i]).error)
			return (err);
		i++;
...
int	ut_strtol_validity(int N)
{
	...
	while (i < N)
	{
		errno = 0;
		s = (char *)(NULL);
		if (strtol((const char *)(s = ((char *)ltostr(i, 10, 0))), (char **)(NULL), 10) != i
			|| errno)
			return (i + 1);
		i++;
	}
