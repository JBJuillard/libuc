...
** ut_strtoll.c
**
** Unit tests for strtoll function of Undefined-C library
**
...
int	ut_strtoll_interface(int N)
{
	...
	while (i < 5)
	{
		...
		if (strtoll((tdat[i]).str, (tdat[i]).end, 10) != (tdat[i]).ret
			|| end != (tdat[i]).endval
			|| errno != (tdat[i]).error)
			return (err);
		i++;
...
int	ut_strtoll_validity(int N)
{
	...
	while (i < (long long int )(N))
	{
		errno = 0;
		s = (char *)(NULL);
		if (strtoll((const char *)(s = ((char *)lltostr(i, 10, 0))), (char **)(NULL), 10) != i
			|| errno)
			return (i + 1);
		i++;
	}
