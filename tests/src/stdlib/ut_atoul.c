...
** ut_atoul.c
**
** Unit tests for atoul function of Undefined-C library
**
...
int	ut_atoul_interface(int N)
{
	...
	{
		errno = 0;
		if (atoul((tdat[i]).str) != (tdat[i]).ret
			|| errno != (tdat[i]).error)
			return (err);
		i++;
		err--;
...
int	ut_atoul_validity(int N)
{
	...
	while (i < (unsigned long int)(N))
	{
		errno = 0;
		s = (char *)(NULL);
		if (atoul((const char *)(s = ((char *)ultoa(i)))) != i || errno)
			return (i + 1);
		i++;
	}
	return (0);
